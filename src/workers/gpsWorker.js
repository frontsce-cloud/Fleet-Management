...
// GPS Worker - only initialize if Redis is available
let gpsWorker = null;

if (!redis || redis.status === 'connecting' || !redis.connected) {
  // Redis not available, skip worker initialization
  console.log('Redis not available, skipping GPS worker initialization');
} else {
  try {
    gpsWorker = new Worker('gps', async (job) => {
    ...
    console.error('Failed to initialize GPS worker:', error.message);
  }
}
